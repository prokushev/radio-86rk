...
!                             !        ! ВЫХОДНЫЕ                        !                 !
+-----------------------------+--------+---------------------------------+-----------------+
! Ввод командной строки       ! 0F8EEH ! ВЫХОДНЫЕ                        !                 !
!                             !        ! CY=1 есть данные, CY=0 пустая   !                 !
...
дисплея, совместимых с терминалом DECscope VT52. В связи с
...
+-------+-------------------------------------------------------------------+
! 1BH+V ! Печать текущей строки на принтер (VT-52)                          !
+-------+-------------------------------------------------------------------+
! 1BH+W ! - не используется, перейти в режим управления принтером           !
+-------+-------------------------------------------------------------------+
! 1BH+X ! - не используется, выход из режима управления принтером           !
+-------+-------------------------------------------------------------------+
...
!       ! - самая левая.                                                    !
+-------+-------------------------------------------------------------------+
! 1BH+Z ! Идентификация типа терминала, возвращает 1BH+/+?, где ?=K,L или M !
+-------+-------------------------------------------------------------------+
...
+-------+-------------------------------------------------------------------+
! 1BH+А ! (РУС) - не используется, Скрыть курсор (расширение M/80K)         !
+-------+-------------------------------------------------------------------+
! 1BH+Б ! (РУС) - не используется, Показать курсор (расширение M/80K)       !
+-------+-------------------------------------------------------------------+
...
+-------+-------------------------------------------------------------------+
! 1BH+] ! Распечатать экран на принтере                                     !
+-------+-------------------------------------------------------------------+
...
Данный набор п/п позволяет полностью перенастроить работу МОНИТОРа под свое оборудование
без изменения прошивки ПЗУ, а также обеспечить совместимость с имеющими программами.
Перехват чтения клавиатуры позволяет установить драйвер
для другого типа клавиатуры или организовать автоввод данных. Перехват подачи звукового
сигнала позволяет поддерживать другое звуковое оборудование, кроме стандартного динамика,
управляемого по INTE. Ввод/вывод на магнитофон позволяет реализовать, например,
загрузку с диска. Вывод на экран может использоваться для поддержки более современного
эмулятора терминала (например, VT-100) или организации оконного менеджера. Ну и п/п
печати позвроляет поддержать свой тип принтера.

## Несовместимость

МОНИТОР 1.20 несовместим по Esc-кодами Esc-a и Esc-b с МОНИТОРом М80/К, т.к. код Esc-b
конфликтует с более распространенным расширением GEM/TOS.
